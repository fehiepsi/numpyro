...
class BarkerMH(MCMCKernel):
    ...
    **Example**
    ...
    def init(self, rng_key, num_warmup, init_params, model_args, model_kwargs):
        ...
        size = len(jax.flatten_util.ravel_pytree(init_params)[0])
    ...
    def sample(self, state, model_args, model_kwargs):
        ...
        x_flat, unravel_fn = jax.flatten_util.ravel_pytree(x)
        x_grad_flat, _ = jax.flatten_util.ravel_pytree(x_grad)
        ...
        y_grad_flat, _ = jax.flatten_util.ravel_pytree(y_grad)
        y_grad_flat_scaled = mass_sqrt_inv @ y_grad_flat if self._dense_mass else mass_sqrt_inv * y_grad_flat

        log_accept_ratio = x_pe - y_pe + jnp.sum(jax.nn.softplus(dx_flat * x_grad_flat_scaled) -
                                                 jax.nn.softplus(-dx_flat * y_grad_flat_scaled))
